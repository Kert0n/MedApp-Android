 ...
 * пачкой и живёт вечно — за неё держатся приёмы и движения, поэтому история читается и после
 * того, как коробка кончилась или выброшена (PLAN D3, D6, D7). Зеркало записи эпизода:
 * тождество общее с живой пачкой, снимок имени, единицы и формы идёт за ней и переписывается той
 * же транзакцией, что и пачка.
 *
 * Исхода у записи нет: есть ли коробка сейчас, отвечает наличие живой пачки, а куда она делась —
 * её движения. Переходов нет тоже: запись меняется только вслед за пачкой ([Package.record]).
 */
class PackageRecord(
    val id: Uuid,
    val name: String,
