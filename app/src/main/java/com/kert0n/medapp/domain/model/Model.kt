 ...
 *
 * Заготовка. Формы взяты у сервера, потому что понятия общие, но клиентский домен не обязан
 * совпадать с серверным: серверу нужно правило, клиенту — что нарисовать и что отправить.
 * Разойдутся они там, где у клиента появится своё: расписание приёмов, черновики, офлайн.
 *
 * Отличие от [провода][com.kert0n.medapp.data.remote.dto] одно: величины здесь `BigDecimal`, а
 * идентификаторы — `UUID`. Строка разбирается один раз на границе, и дальше по коду ходит
 * значение, с которым можно считать.
 */
 ...
 *
 * [drugVersion] обязателен: элемент очереди без версии отправить нельзя — сервер ответит 428.
 * Версия берётся из того чтения, после которого человек начал менять пачку.
 *
 * [claimsVersion] необязателен, и это не послабление: его отсутствие означает, что своей брони
 * ещё не было, — сверяться не с чем, её заводят.
 */
