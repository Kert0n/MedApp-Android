...
class Settlement(val transition: Transition, effects: List<Effect> = emptyList()) {
    ...
    sealed interface Effect {
        ...
        /** Пачки на сервере больше нет: коробки у нас нет, строка уходит со своими частями. */
        data class PackageGone(val packageId: Uuid) : Effect

        /** Доступа к пачке больше нет: последний виденный остаток уходит в историю, строка — с частями. */
