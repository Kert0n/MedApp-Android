...
interface PackageStorageRepository {
    ...
    /**
     * Человек выбросил коробку: уходит вся пачка — её сведения, брони, история остатка и связи с
     * курсами. Это её части, и порознь их не бывает (PLAN D3, F2).
     *
     * Приёмы остаются: что и когда человек принимал — факт лечения, он держится на записи эпизода
     * и на себе самом, а ссылка на пачку пустеет (PLAN D6). `false` — пачки и так нет.
     */
    suspend fun delete(packageId: Uuid): Boolean

    /**
     * Содержимое аптечки переезжает целиком — и живое, и архивное. Это действие над **местом**, а
     * не над каждой коробкой: у пачки перенос требует, чтобы её можно было взять в руки (D3), а
     * здесь человек уносит саму аптечку (PLAN E6). Возвращает, сколько пачек переехало.
     */
    suspend fun moveContents(from: Uuid, to: Uuid): Int

    /** Аптечку выбросили вместе с лекарствами: пачки уходят так же, как от [delete]. */
