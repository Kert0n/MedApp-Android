...
class PackageQueueStateTest {
    ...
    fun confirmedAmountPassesThroughUntouched() {
        ...
        val state = PackageQueueState(tablets("20"))
    ...
    fun consumptionIsSubtractedOnceAndLeavesTheNumberUnconfirmed() {
        val state = PackageQueueState(tablets("20"), listOf(consume("3")))
    ...
    fun queueMarksStayOutOfTheDomainAmount() {
        ...
        val settled = PackageQueueState(tablets("17"))
        val pending = PackageQueueState(tablets("20"), listOf(consume("3")))
    ...
    fun projectedRecountAgreesWithWhatThePackWouldConfirm() {
        ...
        val state = PackageQueueState(stored.quantity, listOf(recount))
    ...
    fun deletionProjectsZero() {
        val state = PackageQueueState(tablets("20"), listOf(PackageSyncCommand.Delete(PACK)))
    ...
    fun negativeProjectionIsShownAsZeroAndNotAsASuccessfulConsumption() {
        ...
        val state = PackageQueueState(tablets("2"), listOf(consume("5")))
        assertEquals(tablets("0"), state.amount.quantityOrNull)
    }
