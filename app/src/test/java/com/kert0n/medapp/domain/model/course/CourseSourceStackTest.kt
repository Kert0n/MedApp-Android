...
class CourseSourceStackTest {
    ...
    fun attachedSourceGoesLastInTheStack() {
        ...
        val withTwo = draftWithDose()
            .attach(home, doses = 5, at = LATER).getOrThrow()
            .attach(dacha, doses = 4, at = LATER).getOrThrow()
        assertEquals(listOf(PACK, OTHER_PACK), withTwo.sources.map { it.packageId })
        assertEquals(listOf(5, 4), withTwo.sources.map { it.allocatedDoses })
        assertEquals(9, withTwo.allocatedDosesTotal)
    }

    @Test
    fun samePackageDoesNotEnterTheStackTwice() {
        val once = draftWithDose().attach(home, doses = 5, at = LATER).getOrThrow()
        val again = once.attach(home, doses = 1, at = LATER)
    ...
    fun unusablePackageIsNotASource() {
        ...
        assertEquals(
            ...
            draftWithDose().attach(archived, doses = 1, at = LATER).rejection()
        )
        assertEquals(
            CourseRejection.PACKAGE_UNUSABLE,
            draftWithDose().attach(lost, doses = 1, at = LATER).rejection()
        )
    }
    ...
    fun reorderMovesPriority() {
        ...
        assertEquals(listOf(4, 5), swapped.sources.map { it.allocatedDoses })
    }

    @Test
    fun reorderOutsideTheStackIsAProgrammerError() {
        val stack = draftWithDose().attach(home, doses = 5, at = LATER).getOrThrow()
    ...
    fun allocationIsUnknownWhileTheDoseIs() {
        ...
        val stack = course().attach(home, doses = 5, at = LATER).getOrThrow()
        assertNull(stack.allocatedOf(PACK))
        assertEquals(5, stack.allocatedDosesTotal)
    }

    @Test
    fun changingSourcesAgesTheRevision() {
        val attached = draftWithDose().attach(home, doses = 5, at = LATER)
    ...
    fun sourcesOfAnActiveCourseAreStillEditable() {
        ...
        val widened = active.attach(dacha, doses = 4, at = LATER).getOrThrow()
        assertEquals(9, widened.allocatedDosesTotal)
    ...
    fun closedCourseKeepsItsSourcesAsHistory() {
        ...
        assertEquals(CourseRejection.COURSE_CLOSED, cancelled.attach(dacha, 1, LATER).rejection())
    ...
    fun completionAndCancellationReleaseWhatIsLeftAllocated() {
        ...
        assertEquals(0, active.complete(LATER).allocatedDosesTotal)
        assertEquals(0, active.cancel(LATER).allocatedDosesTotal)
    ...
    fun activationRequiresScheduleDoseAndSource() {
        ...
        val sourced = dosed.attach(home, doses = 5, at = LATER).getOrThrow()
    ...
    fun draftWithSourcesButNoScheduleIsRejectedForActivationNotForSaving() {
        ...
        val chosen = draftWithDose().attach(home, doses = 5, at = LATER).getOrThrow()
    ...
    @Test
    fun negativeAllocationIsNotRepresentable() {
        assertThrows(IllegalArgumentException::class.java) { CourseSource(PACK, -1) }
    }
