...
`expiresOn.isBefore(date)`, и знак сравнения записан **один раз** — в `ExpiryDate.isExpired`;
там же лежит окно предупреждения `expiresWithin` (пороги D8) и разворот месяца `lastDayOf`.
`PackageFacts.isExpiredOn` и `expiresWithin` ему делегируют.

**Спрашивают правило двое.** Пачка отвечает про себя, а проекция доступного (D4) — про пачку,
которой у неё на руках нет: в `PackageAvailability` лежит только дата. Пока сравнение жило
у пачки, второму спрашивающему пришлось бы повторить `isBefore` у себя — а это ровно тот знак,
который «поправят» при рефакторинге первым. Именно это и позволяет проекции обойтись без сущности
внутри.
...
последним днём, — продуктовое правило `ExpiryDate.lastDayOf` в домене. Причина отказа называется
...
data class PackageAvailability(
    val packageId: Uuid, val expiresOn: LocalDate?, val amount: EffectiveAmount,
...
**Дата не хранится, а спрашивается.** `isExpiredOn(date)` и `expiresSoonOn(date)` принимают дату,
потому что прогнозу нужна дата отчёта, а списку — сегодняшняя; поле `today` внутри проекции
устаревало бы молча. Само сравнение — в `ExpiryDate` (D3).
...
| 14 | `Правило годности живёт в одном месте, а спрашивают его двое`  | `ExpiryDate.isExpired`/`expiresWithin`/`lastDayOf`; `PackageFacts` делегирует (D3)                       |
