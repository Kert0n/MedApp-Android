...
│              и состояние экрана по содержимому; ParsedInput, ScreenState, LoadFailure в корне
...
domain`, **и `network` там, где само действие сетевое**: первичная настройка — это регистрация,
и `feature/bootstrap` зовёт `AccountRegistration` и `VocabularyResolver` напрямую. Сценарий стоит
над всеми пятью логиками, а не сбоку от них. Сценарий `feature/<понятие>` владеет целым действием человека: читает, решает доменом и
...
иначе каждый следующий экран изобретает своё. Причина отказа — значение перечисления, текст к ней
— в `R.string.*`.
...
значением, — это не выбор. Схема задаётся **целиком**: недостающую роль Material подставит из
своих умолчаний, то есть сиреневую.
...
вкладки значок залит — где человек стоит, видно и формой, а не одним цветом.
...
| 4 | `Незарегистрированное приложение честно просит сеть` | `AppStart`, `AppStartState`, экран 1, повтор |
...
Палитра проверяется тем, что отдаёт тема: незаданная роль приходит из умолчаний Material
сиреневой, и видно это только на экране.
