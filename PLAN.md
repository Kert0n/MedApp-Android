...
data class CourseMedicine(
    ...
    fun detach(packageId: Uuid): CourseMedicine
...
**Отвязка последнего источника форму и единицу не сбрасывает ни у `ACTIVE`, ни у `DRAFT`.**
У действующего курса иначе доза и расписание мгновенно потеряли бы смысл, а уже состоявшиеся
приёмы остались бы с единицей, которой у курса больше нет; курс просто становится
необеспеченным. У черновика довод другой и обнаружился позже: доза хранится числом, а единицу
берёт у препарата, поэтому сброс отдавал названную для таблеток двойку следующему препарату —
и «2 таблетки» молча становились «2 миллилитрами». Форма и единица заданы первым препаратом и
описывают само лечение; другой препарат — другое лечение и другой черновик.
...
**`formId = null` не подключается**; **отвязка последнего источника форму не сбрасывает
ни у `ACTIVE`, ни у `DRAFT`**; пропуск освобождает дозу с конца стека; фактическая
