...
| 404 на повторе расхода, опустошившего пачку             | `Applied(Gone)`: сервер списал до нуля и уничтожил пачку — её нет по нашей же причине              |
...
| локальная команда                           | атомарно сохранить данные+команду; отправку после коммита просит `QueueService`, и действие человека её не ждёт |
...
После коммита служба очереди просит отправку — того, кто ставит команды, спрашивать об этом не
нужно, — и обновляются уведомления. Ждать их действию человека незачем: записанное от сети не
зависит, а проход идёт на области жизни приложения и переживает закрытый экран. Падение процесса между коммитом
