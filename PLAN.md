...
@Singleton class QueueOutbox(worker, storage, clock, scope) { fun start(); val state: StateFlow<State> } // единственный владелец drain()
class QueueService(storage) { suspend fun change(medKit: MedKitRef, commands, at, change: suspend () -> Boolean): Boolean }
```
...
известен, и смысла не несёт (разбор PR 13).
...
│   │          QueueWorker, QueueOutbox, QueueService, PackageSnapshotResolver, Settlement
