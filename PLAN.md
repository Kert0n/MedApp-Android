...
| требование | раздел | PR | подтверждается | состояние |
|---|---|---|---|---|
| **Сущности** |
| Препарат: обязательные поля | D3, H3 №7 | 7 | форма отвергает неполное | — |
| Препарат: необязательные поля, все до одного | D3 | 7 | круговой тест формы | — |
| Аптечка: название, идентификатор, число участников, место хранения | D2 | 7 | — | — |
| **Управление аптечкой** |
| Создать, переименовать | H3 №3 | 7 | сценарий J2.1 | — |
| Удалить с удалением содержимого | E6 | 14 | `deleteMedKit` без `targetMedKitId` | — |
| Удалить с переносом содержимого | E6 | 14 | `deleteMedKit?targetMedKitId=` | — |
| Поделиться: QR на весь экран | H3 №21 | 14 | `FLAG_SECURE`, оценка срока | — |
| Поделиться: текстовый код, **автокопирование в буфер** | H3 №20 | 14 | тест буфера обмена | — |
| Предупреждение о необратимости и неотзываемости доступа | E5 | 14 | экран последствий | — |
| **Управление препаратом** |
| Добавление вручную с валидацией | H3 №7 | 7 | невалидное не сохраняется | — |
| Подсказки справочника по названию | H5 | 8 | дебаунс, отмена, кэш | — |
| Добавление сканированием | H5 | 16 | фикстуры EAN-13 и DataMatrix | — |
| Правка полей | H3 №8 | 7 | — | — |
| Изменение количества с разрешением конфликтов | E1, E3 | 7, 13, 15 | пересчёт → пересчёт обеспечения | — |
| Удаление препарата (архивирование) | D3 | 7 | история переживает | — |
| Перенос между аптечками | E6 | 7, 14 | четыре случая переноса | — |
| **Приём** |
| Разовая доза как placeholder, переопределяемый | H3 №10 | 10 | — | — |
| Разовый приём | D6 | 10 | списывает ровно один раз | — |
| Курс: доза, даты, дни недели, времена в дне | D5 | 3, 9 | `ScheduleCalculator` | — |
| Валидация количества под курс | D5, D4 | 3, 9 | **сокращается обеспечение, не курс** (C1) | — |
| Уведомление по достижении срока приёма | D8 | 11 | `AlarmManager`, деградация | — |
| Опрос об успешности приёма, корректирующий количество | D6, H3 №18 | 10, 11 | действия из шторки | — |
| Правка курса: заменяет будущие, не трогает прошлые | D5 | 9 | `revision` | — |
| **Просроченный препарат** |
| Постоянный мониторинг сроков | D8 | 11 | `DailyWorker` | — |
| Уведомление об истечении | D8 | 11 | пороги на фиксированном `Clock` | — |
| Выделение в интерфейсе и **выше остальных** | H3 | 7 | порядок при любой сортировке | — |
| Остаётся до ручного удаления | D3 | 7 | — | — |
| Предупреждение при приёме просроченного | H3 | 10 | — | — |
| **Поиск, фильтр, сортировка** |
| Поиск в аптечке и по всем аптечкам | H4 | 7 | `PackageQuery.medKitId` | — |
| Фильтрация по одному параметру и сортировка | H4 | 7 | все комбинации | — |
| **Порядок действий не влияет на результат** | H4 | 7 | тест перестановок | — |
| Каждая пачка — отдельная единица | A1 | 7 | две одноимённые остаются двумя | — |
| **Личный кабинет** |
| Прогноз количества на дату, не далее трёх месяцев | H6 | 17 | `ForecastCalculator` | — |
| История израсходованного, не более года | H6 | 17 | баланс сходится | — |
| Сводная статистика: категории, формы, общая цена | H6 | 17 | пачки без цены отдельно | — |
| Сводный план лечения на дату | H3 №12 | 10 | — | — |
| **Общая аптечка** |
| Нет различия прав, нет администрирования | — | 14 | свойство сервера | — |
| Удаление по тем же правилам; выход аннулирует курсы | E6 | 14 | выход гасит источники | — |
| Хранится на сервере, клиенты синхронизируются | E4 | 12, 13 | полный снимок | — |
| Курс в общей аптечке бронирует нужное | D5, E2 | 9, 15 | `SetClaim` | — |
| Предупреждение при разовом использовании забронированного | D4 | 10, 15 | `freeForAnyone` | — |
| Забронированное нельзя взять в другой курс | F1 | 9 | `active_package_assignments` | — |
| **База препаратов** |
| Справочник с названием и формой; прочее необязательно | B4, H5 | 8 | `DrugTemplateDTO` | — |
| **Конфликты** |
| Разрешение конфликта количества | E3, D5 | 13, 15 | сценарий J2.3 | — |
| Уведомление об изменении обеспечения | D8 | 11, 15 | `COVERAGE_SHORT` | — |
| **Входные данные** |
| Проверка типов, длин, бизнес-логики | D1, H3 | 2, 7 | `Quantity.parse`, лимиты полей | — |
| «Реалистично некорректные» данные принимаются и отрабатываются | D3 | 7 | просроченная дата вводится | — |
| **Интерфейс** |
| Десять обязательных экранов | H3 | 6–17 | навигационный тест | — |
| Подтверждения опасных действий | H3 | 7–14 | список из девяти | — |
| **Надёжность** |
| Не падает ни на каком вводе | D1, H2 | 2, 5 | `parse` возвращает `Result`; битый JSON | — |
| **Совместимость** |
| Android 8.0 и выше | H2 | 1 | запуск на API 26 | — |
| Камера обязательна для сканирования | H5 | 16 | отказ ведёт к ручному вводу | — |
