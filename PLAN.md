...
Разбор живёт в `ExpiryDate.parse`, рядом с упаковкой: правило включительной даты доменное, и
проверяется оно доменным тестом. Причина отказа называется, чтобы «13.2027» и «март 2027» не
превращались в одинаковое «неверный формат».
...
**Изменяющие переходы требуют `ACTIVE`.** `consume`, `correctTo`, `describe` и `moveTo` над
`ARCHIVED` или `INACCESSIBLE` отвергаются. `ARCHIVED` означает «израсходована, утилизирована или
удалена человеком»: отменять осознанное удаление новым числом нельзя, поэтому **пересчёт архив не
оживляет**. Недоступную пачку не правят и не переносят — её у нас нет. Возврат из архива, если он
понадобится, будет отдельным явным действием со своим подтверждением, а не следствием пересчёта.
`archive` и `loseAccess` идемпотентны; архивировать недоступную пачку разрешено (так её убирают из
списка), а обратный переход запрещён — он ничего не добавляет к истории и спрятал бы пачку из неё.
