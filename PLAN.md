...
| Серверное хранение расписаний, приёмов, курсов | прямо противоречит границе данных (A2) |
...
/** То же для PATCH: null = «не трогать», "" = «очистить». */
data class PackageWireEdit(
    val name: String? = null, val quantity: Quantity? = null, val formId: Uuid? = null,
    val category: String? = null, val manufacturer: String? = null,
    val country: String? = null, val description: String? = null
)

/** Правка всех полей, включая локальные. null = «не трогать», "" = «очистить». */
...
и состояние, и уведомление — с той же дедупликацией, что обычная просрочка.
...
| подтвердили приём | пачка уменьшилась на дозу, **выделение — тоже**, бронь — на ту же величину |
...
Отсюда `allocatedDoses` — это **невыбранный остаток выделения**, а не изначально назначенное
число. Он уменьшается с каждым подтверждённым приёмом; ровно его умножение на дозу и есть величина
брони. Изначальное назначение нигде не хранится: восстановить его можно по приёмам курса, а
держать вторым числом значило бы завести второй источник правды, который разъедется.

**Почему одна упаковка не может попасть в два незавершённых курса.** Бронь на сервере
идентифицируется парой `(userId, drugId)` — первичным ключом таблицы. Второй независимой брони на
ту же пачку у того же человека не бывает, и два курса не смогли бы выразить свои выделения по
отдельности. Правило совпадает с тем, как устроено лечение (параллельный приём одного препарата
согласуется вне приложения), но держится не соглашением, а контрактом и первичным ключом
...
Обеспечение пересчитывается в трёх местах, и только в них: **после применения снимка**, **после
подтверждения приёма**, **после правки курса**. Для затронутого источника:

```
новое выделение (в дозах) = min(текущее выделение, availableToMe.dosesIn(dose))
```

**Расписание, доза и даты курса не меняются никогда.** Уменьшается обеспечение, и человеку
показывается, сколько приёмов нужно, сколько и чем обеспечено и с какого приёма начинается
нехватка.

Это **не алгоритм дележа дефицита**: контракт не передаёт чужие брони поимённо, кто и сколько
заявил — неизвестно, и клиент правит только своё выделение.

**Правка курса не увеличивает броней сама по себе.** Удлинение курса или увеличение дозы создаёт
нехватку и показывает её; выделение вырастет только после того, как человек подвинет ползунок.
...
data class Intake(
    ...
    /**
     * Расход учтён там, где лежит истина: в локальной аптечке это Room и признак ставится
     * сразу, в опубликованной — ответ сервера. False значит ровно одно: операция закрыта
     * как SETTLED, и применялась ли она — неизвестно (E3).
     */
...
data class PlannedNotification(
    ...
    val channel: String,                 // идентификатор канала из таблицы ниже
...
sealed interface SyncIntent {
    ...
    /**
     * Расход. claimAfter — величина брони ПОСЛЕ приёма: у курсового приёма это выделение,
     * уменьшенное на дозу; у разового вне курса — null.
     */
...
**Какой эндпойнт выбирает `Consume`.** Это единственное место, где два маршрута контракта решают
одну задачу, и путать их нельзя:

| случай | запрос | почему |
|---|---|---|
| приём по курсу (`claimAfter != null`) | `PUT /v1/drugs/{id}/sync/{syncId}` с `consumed` и `reservation` | расход и бронь обязаны примениться **вместе**. Разделив их на два запроса, между ними получаем состояние, где пачка уже уменьшилась, а бронь ещё держит прежнее — и другой участник видит меньше свободного, чем есть. Ради этой атомарности маршрут и сделан |
| приём вне курса (`claimAfter == null`) | `POST /v1/drugs/{id}/intakes` | брони нет, менять нечего |
| последняя доза источника | `sync` с `consumed`, **без** блока `reservation`, затем `ReleaseClaim` | `reservation.amount` строго положителен, обнулить бронь через `sync` нельзя (B6) |

`syncId` — это `SyncOperation.id`, и он не меняется между попытками. Тот же идентификатор с другим
содержимым сервер считает конфликтом, поэтому пересобирать тело при повторе нельзя вдвойне.
...
Room и без сети. Данные приходят `Flow`-ом из Room (правило A3.1), поэтому «обновить экран после
...
| 11 | `Нехватка зажимает выделение, не трогая расписания` | формула пересчёта и три места, где он запускается |
| 12 | `Прогноз считается формулой, а не строками` | `ForecastCalculator` |
...
выделение и **не трогает расписания, дозы и дат**; удлинение курса создаёт нехватку, но брони не
увеличивает.
...
**своя бронь без локального курса показывается человеку, а не снимается молча**.
...
| 7 | `Подтверждённое и урегулированное — разные состояния` | `DONE` против `SETTLED` |
| 8 | `Неподтверждённый расход виден человеку` | `serverConfirmed`, экран 28 |
| 9 | `Пересчёт снимает неопределённость, не списывая второй раз` | разрешение через `PatchPackage` |
...
`payloadVersion` даёт `CONFLICT`, а не падение; **курсовой приём уходит одним `sync` с расходом и
бронью, разовый — через `intakes`**; **тот же `syncId` с изменённым телом даёт 409**; последняя доза
источника снимает бронь отдельным `DELETE`; отсрочка повторов растёт экспоненциально и подчиняется
`Retry-After`.
...
| **Границы продукта** |
| Взаимозаменяемость, подбор лечения, пересчёт единиц — **не делаем** | C2 | — | раздел «чего нет» | ✔ |
| **Конфликты** |
