...
| **Удаление упаковки**          | «удалить из базы»                                | **удаление** строки со всеми её частями; `ARCHIVED` остаётся за «кончилась или выброшена» | решение 2026-09-12 **отменяет прежнее** «архивирование вместо удаления»: опора у истории лечения другая — запись эпизода и сами приёмы, а не картонная коробка. Ссылка приёма на удалённую пачку пустеет (D6) |
| **Удаление аптечки**           | ТЗ 4.1.1.2.3: с лекарствами или с переносом      | оба пути местные и в одной транзакции; общая — только сервером                     | решение 2026-09-12: местная аптечка существует только у нас, и её удаление связи не требует (C3). Переезжает всё содержимое, включая кончившиеся пачки: уносят место, а не каждую коробку |
...
| **Публикация**                 | `PUBLISHING`, группа операций, приём во время публикации не теряется | только при связи; до переключения у серверной копии один писатель — устройство: публикуется текущее состояние, обрыв продолжается по идентификаторам, откат `DELETE` — только на отказ; переключение и первые остатки — одна транзакция | решение 2026-09-11: откат на обрыве делал потерянный ответ «откатом», а повтор — вечным 409. Местная аптечка приглашений не выдаёт, поэтому «картина начала» не нужна |
...
class MedKit(
    ...
    fun publish(): MedKit                       // LOCAL → PUBLISHED, одним переходом
...
**Состояний публикации два, и промежуточного нет.** Публикация — одно действие при связи:
`MedKitPublication` создаёт аптечку и все её пачки подряд, и если любой шаг отказал или оборвался,
уже созданная аптечка удаляется `DELETE` (E5). Половина — состояние, в котором правка пачки уезжает
к тому, чего не существует, а приглашённый видит часть аптечки, — не выражается типом, поэтому
`acceptsInvitations` выводится из тех же двух состояний, а в очередь публикация не ставится.
...
**Упаковка удаляется по-настоящему** (ТЗ 4.1.1.3.5, решение C1 от 2026-09-12). Уходит вся она: её
сведения, брони, история остатка и связи с курсами — это части пачки, и порознь их не бывает;
держит это схема ключами `CASCADE`, а не порядок вызовов (F2). Историю лечения это не рушит: она
держится на записи эпизода и на самих приёмах, а ссылка приёма на исчезнувшую пачку пустеет (D6).
...
или утилизирована»: отменять осознанный конец новым числом нельзя, поэтому **пересчёт архив не
оживляет**. Удаление — не это: удалённой пачки не существует вовсе, и строки после неё не
остаётся. Недоступную пачку не правят и не переносят — её у нас нет. Возврат
...
data class TakenDose(
    val pkg: PackageRef?,    // может отличаться от плановой; null — пачку удалили, а приём был
    val amount: Dose,        // не ноль: принятый ноль — это не приём
    val at: Instant
)

// Приём переживает удаление пачки: количество и момент записаны в нём самом, ссылка пустеет
// (SET NULL в схеме, F1). В момент записи пачка есть всегда — её приносит take.

// domain/pack — акт «беру из этой пачки»: пачка годна (PACKAGE_UNUSABLE), единица та (UNIT_MISMATCH);
// пачка курса (PACKAGE_NOT_A_SOURCE) — правило курса, и проверяет его сценарий до акта
...
**Ключи на упаковку различают части и факты.** Движение — часть пачки: `ON DELETE CASCADE`, без
неё запись не значит ничего. Приём — факт лечения: `ON DELETE SET NULL`, ссылка пустеет, а
количество, момент и эпизод остаются (D6). Запись эпизода держит приёмы `RESTRICT`: её не
удаляют вовсе.
...
|------------|----------|-------------------------------------------|
| `intakes`                     | поля `CourseIntake` и `UnplannedIntake` в одной таблице, вид различается наличием курса; `planned_package_id?`, `taken_package_id?` — пачки, в домен собираются связями; `accounting`, `operation_id` — обвязка синхронизации (`IntakeSyncState`); UNIQUE(`course_id`,`scheduled_on`,`scheduled_time`); FK на плановую/фактическую `packages` **SET NULL**, FK на `course_records` **RESTRICT** | Приём переживает удаление пачки: ссылка пустеет, факт остаётся (D6). Аптечки на момент события у факта нет — аптечка у пачки. Колонки учёта живут в той же строке, но доменная модель их не носит |
...
В локальной аптечке исходящих операций нет. Публикация в очередь не ставится: она идёт при связи и
продолжается по идентификаторам (E5), а первое подтверждённое число опубликованной пачки — ответ
сервера на её создание или правку, записанный вместе с переключением аптечки.
...
| 409 на `Create`                                         | `GET`: наша — `APPLIED`                                                                             |
...
Дальше — одно действие при связи, `MedKitPublication`: `POST /v1/med-kits`, затем
`POST .../drugs` на каждую пачку. В очередь публикация не ставится — без связи она не начинается.

**До переключения у серверной копии один писатель — это устройство**: местная аптечка приглашений
не выдаёт (PR 14). Поэтому «картины начала» нет — публикуется **текущее** местное состояние, и что
человек делал между попытками, попадает на сервер той попыткой, которая дойдёт до конца.

Тождество попытки — идентификаторы. Аптечка уже есть (409, потерянный ответ) — `GET`: наша —
продолжаем, 404 — на сервере ничего нет. Пачка уже есть — `GET`, и расхождения с местной уезжают
`PATCH` по прочитанной версии. Серверная пачка, которой местно больше нет, удаляется. Обрыв не
откатывается — повтор доводит; `DELETE` аптечки откатывает только отказ (400, 403): после него на
сервере либо всё, либо ничего. Не прошёл и откат — `Refused(rolledBack = false)`, повтор доведёт.

**Пачки читает и ответ пишет один владелец.** Между чтением списка пачек и `published()` человек
продолжает жить: принял дозу, пересчитал остаток. Если переход запишет старый серверный снимок
поверх нового местного расхода, расход исчезнет. Поэтому владелец перехода читает пачки и пишет
ответ в одной ответственности: пачки, изменённые между чтением и записью, публикуются заново или
откатывают переключение. Сценария публикации ещё нет — правило принадлежит его владельцу (PR 14).

**Момент передачи ответственности — транзакция `MedKitStorageRepository.published`**: переключение в
`PUBLISHED` и ответы на создание пачек — первый подтверждённый остаток и версии (E1). До неё истина —
устройство и очереди нет; после — сервер, и все изменения идут командами. Серверная аптечка с моим
`id`, которую устройство считает местной или не знает, — незавершённая публикация или её сирота;
правило реализуется с полным снимком (E4).
...
**Местная аптечка убирается без сети**, и это два пути ТЗ 4.1.1.2.3: «забрать» — всё содержимое,
живое и кончившееся, переезжает в названную аптечку, после чего исходная удаляется; «выбросить
вместе с лекарствами» — пачки уходят со всеми своими частями. Оба пути — одна транзакция
(`MedKitRemoval` в `feature/medkits`), и лечение их переживает: запись эпизода и приёмы остаются,
ссылка приёма на исчезнувшую пачку пустеет (D3, D6). Общая аптечка местным решением не убирается —
она существует у других людей.

Два пользовательских сценария **общей** аптечки: **«Выйти»** (аптечка остаётся остальным) и **«Забрать аптечку»**
...
|-----------------------|-------------------------------------------------------------------------------|
| локальная → локальная | одна транзакция Room; записи в истории нет — перенос ничего не тратит (D7)     |
...
| `claims`                      | `package_id` PK/FK **RESTRICT**, `total`, `mine?`                                                                                                                                              | Отдельно: картину броней двигают чужие действия. Версия картины — предусловие, она в колонках `packages` вместе с остальной обвязкой синхронизации, и двигает её только половина броней своим `UPDATE` (E1)                                                                                                                                          |
...
| `intakes`                     | поля `CourseIntake` и `UnplannedIntake` в одной таблице, вид различается наличием курса, **без колонок учёта**; `accounting`, `operation_id`, `reconciliation_id` — обвязка синхронизации (`IntakeSyncState`); UNIQUE(`course_id`,`scheduled_on`,`scheduled_time`); FK на плановую/фактическую `packages` **SET NULL**, FK на `course_records` **RESTRICT** | Приём переживает удаление пачки: ссылка пустеет, факт остаётся (D6). Колонки учёта живут в той же строке, но доменная модель их не носит: их меняют только транзакционные сценарии F5, и правила о приёме их не читают |
| `stock_adjustments`           | поля `StockMovement`; FK на `packages` **CASCADE**                                                                                                                               | Одна строка на движение — запись о пачке: аптечек в ней нет, вида «перенос» тоже (D7). Движение — часть пачки и уходит вместе с ней                                                                                                                                                        |
...
**Поведение ключа называет отношение, а не осторожность.** `CASCADE` — у частей: сведения пачки,
брони, движения, источники курсов и назначения без своей пачки не существуют. `SET NULL` — у
фактов: приём случился, и удалённая пачка не отменяет его — пустеет только ссылка (D6).
`RESTRICT` — там, где удаление должно быть невозможным: запись эпизода, единица и форма словаря,
аптечка под живыми пачками. Каскад, поставленный «на всякий случай», незаметно уносит историю;
`RESTRICT`, поставленный туда же, незаметно запрещает человеку выбросить коробку.
...
Версия схемы — **2**: переход 1→2 (`MedAppDatabase.MIGRATION_1_2`) добавил `sync_operations.outcome_unknown`
(E3); старые строки получают «исход известен».
...
|---------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------|
| приём (`IntakeConfirmation`)          | эпизод открыт, условный переход статуса, фактические пачка/единица, расход из нынешнего остатка, выделение источника, accounting; команду расхода ставит `QueueService` в той же транзакции; движения приём не пишет (D7) |
...
| пересчёт / утилизация / перенос       | движение (у переноса его нет), остаток/проекция, состояние упаковки, выделения, исходящие операции                                    |
| удаление аптечки (`MedKitRemoval`)    | перенос всего содержимого **или** удаление пачек со всеми их частями, затем строка аптечки; приёмы остаются, их ссылка на пачку пустеет |
| публикация (`published`)              | переключение аптечки в `PUBLISHED` и первые подтверждённые остатки и версии её пачек                                                  |
...
│   │          резолвер словаря, публикация, обвязка предусловий; про очередь не знает
...
│   │          Settlement
│   ├─ pack/, medkit/ — команды и подготовка запроса по прочитанному состоянию
...
└─ feature/    bootstrap/ (AppStart, AppStartState, AppStartViewModel, SetupScreen), medkits/,
               packages/, courses/, schedule/, intake/, sharing/, analytics/, scanner/, settings/, syncstatus/
```
...
### Локальный учёт: набор экранов 2–9, 11 (PR 7)

Описан целиком, а не по экрану: сквозные решения — где живёт поиск, куда ведёт действие, чем
строка списка отличается от карточки — на одном экране не принимаются, а раздробленный набор
оставляет функции без места.

```
[вкладка Аптечки]
   └─ Аптечки (2) ── поиск по названию ──→ Все лекарства (5)   ← по всем аптечкам
        ├─ (+) ─────────────────────────→ Аптечка: форма (3)
        └─ нажать аптечку ──────────────→ Содержимое (4)        ← поиск в этой аптечке
                                             ├─ (+) ──────────→ Упаковка: форма (7)
                                             ├─ ⋮ ────────────→ правка (3) · удаление аптечки
                                             └─ нажать пачку ─→ Карточка (6)
   Все лекарства (5) ── нажать пачку ────────────────────────→ Карточка (6)
                                                                  ├─→ Правка (8)
                                                                  ├─→ Пересчёт и утилизация (9)
                                                                  └─→ Перенос (11)
```

**Поиск — одно чтение в двух областях** (`PackageQuery.medKitId`): на экране 2 поле ищет лекарства
**по всем** доступным аптечкам и показывает результат экраном 5; на экране 4 — в этой аптечке.
Отдельной вкладки «Все лекарства» нет: обязательным экраном ТЗ её не называет, а требование —
именно поиск в двух областях.

**Вход есть у каждого:** 2 — вкладка; 3 — кнопка на 2 и меню на 4; 4 — нажатие аптечки; 5 — поиск
на 2; 6 — нажатие пачки на 4 или 5; 7 — кнопка на 4; 8, 9, 11 — действия карточки; удаление
аптечки — меню на 4.

| экран | что показывает | состояния |
|---|---|---|
| **2** Аптечки | `Card` на аптечку: название; содержимое — «12 упаковок» или «пусто», при просрочке значок и «2 просрочены» цветом `error`; место хранения; у общей значок и «Общая · 3 участника» | чтение не пришло · ни одной аптечки (рассказ, что это, и кнопка) · список с `FAB` |
| **3** Аптечка: форма | название и место, счётчики длины, отказ под полем; кнопка сохранения не гаснет — погашенная не объясняет, чего не хватает | заведение · правка |
| **4** Содержимое | шапка с названием и меню; `SearchBar`; `FilterChip`-ы (просроченные · истекают · на курсе · категория · форма); выбор сортировки. `Card` на упаковку: название; остаток и «свободно 15» янтарным, если занято; срок «до 03.2027», просроченный красным со значком и словом | пусто в аптечке (кнопка добавить) · **ничего не нашлось** (сброс) — это разные сообщения |
| **5** Все лекарства | тот же список с `medKitId = null`; у строки добавляется, из какой аптечки | как у 4 |
| **6** Карточка | `Card`-ами: сколько есть (остаток крупно, ниже «доступно мне» и «свободно для всех», если отличаются; чужие брони янтарным); что это (только заполненное); сроки и цена; где лежит. Действия: пересчитать, утилизировать, перенести, править | — |
| **7/8** Упаковка: форма | сверху четыре обязательных поля (C1): аптечка, название, количество, единица; ниже раскрываемое «Необязательное» — **все** прочие поля. В правке количество заменено строкой «20 таблеток · изменить» и ведёт на 9 | заведение · правка |
| **9** Пересчёт и утилизация | две вкладки: «пересчитал» (новое количество целиком) и «выбросил» (сколько и почему) | подтверждение, когда выходит ноль |
| **11** Перенос | локальные аптечки, кроме нынешней; общие не предлагаются — перенос в общую требует связи (C3), и список говорит это строкой, а не молчит | — |
| удаление аптечки | пустая — подтверждение; непустая — перенести содержимое или удалить вместе с ним, оба названы последствиями | оба с подтверждением |
...
решение C1 от 2026-09-12). «Добавленное» — приходы; «оставшееся» — нынешние остатки; пересчёты и
утилизация видны отдельными строками, потому что объясняют, **куда** делось.
...
`Delivery.settlement(command)`: переход строки операции и эффекты — положить снимок, обнулить
и заархивировать пачку, снять доступ, учесть приём, закрыть зависимых. Хранение применяет
...
просит «истрачено = добавленное − оставшееся», а разрез по местам был выдумкой плана. Строка PR 4
«История не удаляется вместе с упаковкой» тем самым сузилась: не удаляется **лечение**, а история
остатка — часть пачки и уходит с ней.

**Тесты:** приём читается после удаления пачки и называет количество и момент; движения уходят
вместе с пачкой; курс остаётся без выброшенного источника; аптечка с переносом сохраняет пачки, в
том числе кончившиеся; аптечка с содержимым не оставляет ни пачек, ни их истории остатка, но
оставляет запись эпизода и приёмы; общая аптечка отвергается; миграция 2→3 сохраняет остальные
движения и роняет переносы.
...
| 2 | `Публикация идёт при связи и целиком`                                         | экран ведёт `MedKitPublication` (PR 9–10), исход пишет `published`         |
...
| REQ-005 | Удалить с удалением содержимого                                     | ТЗ 4.1.1.2                              | D3, E6     | #17, 14   | местная — `MedKitRemoval` одной транзакцией; общая — `deleteMedKit` без `targetMedKitId` | PR #17 (местная) |
| REQ-006 | Удалить с переносом содержимого                                     | ТЗ 4.1.1.2                              | E6         | #17, 14   | местная — перенос всего содержимого и удаление; общая — `deleteMedKit?targetMedKitId=` | PR #17 (местная) |
...
| REQ-015 | Удаление препарата                                                  | ТЗ 4.1.1.3; уточнение C1 от 2026-09-12  | D3         | #17, 7    | строка уходит со своими частями; лечение переживает | PR #17 (хранение) |
