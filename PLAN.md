...
`recordIntake` клиент не зовёт: расход — и курсовой, и внеплановый — идёт `synchronise`, у которого
есть клиентский номер и журнал (C1, E3). Путь и `PackageConsumeNetworkDTO` остаются в `network/`
до [MedAppServer#145](https://github.com/Kert0n/MedAppServer/issues/145). 409 у `synchronise` сервер отвечает и на устаревшую версию, и на другое тело под тем же
номером; второе клиент исключает инвариантом переподготовки (E3), различать их просит тот же issue.
...
| Журнал идемпотентности `sync` — **Caffeine в памяти процесса**, 24 часа, теряется при перезапуске, не разделяется между экземплярами                    | Повтор с неизвестным исходом узнаётся журналом в пределах суток; сутки без связи ∧ перезапуск ∧ чужое изменение — принятый остаток риска (E3); журнал в базе просит MedAppServer#145                                                                                           |
...
один не блокирует работу, каждый стоит клиенту лишнего запроса или менее точного текста. Журнал `sync` в базе, различимый 409
и `intakes` с клиентским номером заведены отдельно — [MedAppServer#145](https://github.com/Kert0n/MedAppServer/issues/145).
...
| **Отказ по версии на расходе** | 409 у `sync` — «уже применено»                 | «не применилось»: снимок, подготовка заново **под тем же `syncId`**, меняются только версии; у курсового применение видно по `mine` | сервер отвергает до применения; журнал сравнивает `consumed` и `reservation.amount`, версии — нет |
...
| **Остаток риска**              | —                                                | сутки без связи ∧ перезапуск сервера ∧ чужое изменение — принят; исход виден расхождением и чинится пересчётом | сервер не списывает ниже нуля, число на экране — серверное; журнал в базе просит [MedAppServer#145](https://github.com/Kert0n/MedAppServer/issues/145) |
| **Расход — всегда `sync`**     | курсовой — `sync`, внеплановый — `POST .../intakes` без идентификатора | `Consume` всегда `PUT .../sync/{syncId}` | у `sync` есть клиентский номер и журнал; путь `intakes` остаётся в `network/` до MedAppServer#145 |
...
interface QueueStorage {           // реализует хранение
    ...
    suspend fun take(id: Uuid, fresh: PackageSnapshotNetworkDTO?, at: Instant): Take? // применить прочитанное, подготовить, SENDING
    ...
    suspend fun settle(id: Uuid, outcome: Delivery, at: Instant) // статус, снимок, учёт приёма, каскад зависимых
...
sealed interface Delivery { Applied(state); Stale(snapshot); Refused(reason, snapshot?); Retry(reason, notBefore); AccessLost }
@Singleton class QueueWorker(storage, transport, vocabulary, clock) { suspend fun drain(): Report } // один исполнитель
...
409 у `sync` сервер отвечает на две причины — «версия не текущая» и «другое тело под тем же
номером»; вторую исключает инвариант переподготовки (тело неизменно), а различать их просит
[MedAppServer#145](https://github.com/Kert0n/MedAppServer/issues/145). Создание по клиентскому идентификатору повторно даёт 409 (B4).

**Остаток риска назван и принят** (решение владельца 2026-09-11): сутки без связи ∧ перезапуск
сервера (журнал в памяти, B6) ∧ чужое изменение пачки. Исход бывает только видимым — расхождением
...
| 409/412 на `Consume`, `SetClaim`, `ReleaseClaim`        | `Stale`: снимок, переподготовка под тем же номером; у `Consume` сначала улика `mine`               |
| 409/412 на `Describe`, `CorrectStock`, `Move`, `Delete` | `Refused(STALE)` со снимком: чужая правка перекрыла намерение, человек видит новое состояние        |
...
| нет соединения                                          | `Unavailable`: проход останавливается, `attempts` не растут                                        |
...
### RK-SYNC-01 — сужено до названного остатка

Случай A из [MedAppAndroid #2](https://github.com/Kert0n/MedAppAndroid/issues/2) — расход применился, ответ потерян, пачку изменили — закрыт: расход идёт `sync`,
повтор тем же `syncId` узнаёт журнал, у курсового — ещё и `mine`. Случай B — расход не дошёл, пачку
изменили — закрыт переподготовкой по прочитанному: расход применяется, а не отвергается. Остаётся
названный выше остаток риска — сутки без связи ∧ перезапуск сервера ∧ чужое изменение; журнал в
базе просит [MedAppServer#145](https://github.com/Kert0n/MedAppServer/issues/145).
...
| исход операции (`QueueStorage.settle`)| статус операции, снимок пачки и брони, учёт факта (`REMOTE_APPLIED`/`REMOTE_REFUSED`), каскад зависимых; ответ записан раньше (`answered`)                                 |
...
│   │          PackageQueueState, QueuedCommand, QueueStorage, QueueTransport, QueueHttpTransport,
...
идемпотентность регистрации — [MedAppServer#143](https://github.com/Kert0n/MedAppServer/issues/143);
...
Отказ по предусловию закрывает операцию и читает снимок; 409 у `sync` — «устарело»: чтение и
подготовка заново под тем же `syncId`;
