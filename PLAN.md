...
| **Повтор по остатку**          | эвристика по прочитанному остатку, замороженный повтор, ручная сверка | **повтор тем же запросом**: применилось — 412 и снимок, не применилось — применится; статуса неопределённости нет | решение разбора PR 9: устаревшее предусловие отвергает сервер. Неверное состояние — факт без серверного расхода после чужого изменения — остаётся и описано в #2 |
...
целиком (E5), и первое подтверждённое число опубликованной пачки — ответ сервера на её создание.
...
### RK-SYNC-01 — обработано, но не исчезло

Прежняя эвристика «прочитанный остаток больше ожидаемого → расход не дошёл» снята: повтор идёт
замороженным предусловием, и применённый расход второй раз сервер не примет — версия уже другая.
Само неверное состояние осталось и описано заново в
[MedApp-Android #2](https://github.com/Kert0n/MedAppAndroid/issues/2): после потери ответа на
`POST intakes` и чужого изменения пачки клиент не отличает «применилось, потом версия ушла» от
«не дошло», и в обоих случаях закрывает операцию отказом; во втором факт приёма и серверный
остаток расходятся, и никто об этом не подсказывает. Курсовой `sync` этой неоднозначности не
имеет — журнал идемпотентности под `syncId` держит сервер; кандидат на решение — вести через него
и внеплановый расход. Решение — за владельцем.
...
`POST .../drugs` на каждую пачку. Отказ или обрыв на любом шаге откатывается `DELETE` аптечки:
на сервере она либо целиком, либо её нет, и состояние `PUBLISHING` ничему не соответствует (D2).
Если не прошёл и откат, человеку показывается, что связи нет; повторная публикация ответит 409 по
идентификатору аптечки, и тогда её читают снимком. В очередь публикация не ставится — без связи она
не начинается. Ответы на создание пачек — первый подтверждённый остаток и версии (E1).
...
- **Ключ, который не удалось записать**, — отдельный исход регистрации, а не исключение: сервер
  ...
  она отдельно — `Kert0n/MedApp-Server#143`.
...
| 7 | `Принятый риск синхронизации отслеживается отдельно`  | `MedApp-Android #2`: `RK-SYNC-01` — переписан после разбора PR 9: обработан, но не исчез (E3)               |
...
| 2 | `Публикация идёт при связи и целиком`                                         | экран ведёт `MedKitPublication` (PR 9), записывает исход         |
...
откатывается `DELETE`, и аптечка остаётся местной; **местная аптечка приглашений не выдаёт**;
недействительное
