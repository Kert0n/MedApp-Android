...
`OfflineSyncStoriesTest` — пустой `200` после полного `sync`, сценарии удаления/выхода — пустой `204`.
...
| Словари и справочник | данные каталога | кэш и встроенный снимок словарей с происхождением |
...
`availableToMe` — локальная оценка по последним сведениям, а не гарантия серверной блокировки запаса.
...
sealed interface StockViewState {
    ...
    data class NeedsRecount(val lastObserved: Quantity, val unresolvedOperationIds: List<Uuid>) : StockViewState
}
...
data class Course(
    ...
    fun allocate(packageId: Uuid, doses: Int): Course        // предел вычисляется use case по свежему состоянию
...
расход и уменьшение брони совпадают; при частичной/увеличенной фактической дозе бронь пересчитывается
по правилам ниже и может уменьшиться на другую величину. `sync` передаёт именно два полученных числа.
...
Несколько черновиков могут предварительно выбирать одну пачку; конкуренция разрешается при активации.
...
Пользовательское «удалить курс» не удаляет факты из базы. Источники, их порядок и выделения активного
курса менять можно; это не изменение назначенной дозы или календаря. Название и заметка редактируются.
...
или единицу пачки, несовместимый источник отключается с объяснением, а доза/единица курса не меняются.
...
Пример: 5 доз по 2, принято 1 → расход 1, осталось 4 целых выделенных дозы (8 единиц), 1 освобождена.
...
проверки. Идемпотентность — условный `UPDATE` по ожидаемому статусу; повтор уже совершённого действия
...
**Причина не выдумывается.** `REMOTE_CHANGE` показывается как «изменение общего остатка, причина неизвестна», без
...
data class NotificationKey(val kind: NotificationKind, val subject: String) {
    ...
    companion object {
        ...
        fun coverage(courseId: Uuid, firstUncoveredAt: Instant,
                     kind: NotificationKind): NotificationKey
...
просрочке или затрагивании броней открывается форма; предупреждение не обходится действием из шторки.
...
sealed interface SyncIntent {
    ...
    data class Consume(val packageId: Uuid, val amount: Quantity, val intakeId: Uuid,
                       val claimAfter: Quantity?) : SyncIntent
    ...
    data class ReconcileStock(val packageId: Uuid, val actual: Quantity,
                             val throughSequence: Long) : SyncIntent
}
...
**Намерение приёма не поглощает следующий факт.** Каждое подтверждение имеет отдельный идентификатор.
...
`SyncOperationFactory` учитывает исходную и целевую аптечки, фазу публикации и наличие незавершённого
...
условиях — свидетельство, а исчезновение брони, изменение её общей версии или просто «она другая» — нет.
...
- соответствующие факты сохраняются и связываются со сверкой, их старый сетевой исход не меняется на «доказанный»;
...
Повторные триггеры объединяются одним координатором; независимые пачки могут отправляться параллельно,
одна пачка — последовательно. Фоновая задача не обещает точного времени. Ошибка чтения не стирает кэш;
...
и предупреждает: **это необратимо, переданный доступ нельзя отозвать**. Любой участник может удалить аптечку
...
Второй описывает физический перенос: забрал содержимое с дачи домой. Отдельно доступно явное удаление
...
- Подтверждение одного пункта расписания неповторимо: `intakes.id` — первичный ключ, и подтверждение
  идёт условным `UPDATE` по ожидаемому статусу (`PLANNED` либо явно подтверждаемый `MISSED`), а не вставкой.
...
создавать их разом при сохранении незачем. После долгого простоя пропущенные календарные пункты достраиваются как `MISSED` по тем же
...
data class PackageForecast(val packageId: Uuid, val at: Instant, val remaining: Quantity?,
...
interface ScheduleCalculator {
    ...
    fun occurrences(schedule: CourseSchedule, from: Instant, until: Instant): List<ScheduledOccurrence>
    fun countRemaining(schedule: CourseSchedule, from: Instant,
                       resolved: Set<Pair<LocalDate, LocalTime>>): Int
}
interface AllocationLimits {
    fun maxDoses(packageId: Uuid, course: Course, requiredDoses: Int,
                 availability: Map<Uuid, Quantity>): Int
}
interface CoverageCalculator {
    ...
    fun coverage(course: Course, remaining: List<ScheduledOccurrence>,
                 availability: Map<Uuid, Quantity>): CourseCoverage
    fun assignSources(course: Course, upcoming: List<Intake>,
                      availability: Map<Uuid, Quantity>): Map<Uuid, Uuid?>
}
interface ForecastCalculator {
    // Дата включительно в reportZone; горизонт не больше трёх календарных месяцев.
    fun remainingOn(date: LocalDate, reportZone: ZoneId, now: Instant,
                    packages: List<PackageView>, courses: List<Course>,
                    resolved: List<Intake>): List<PackageForecast>
}
```
...
`encodeDefaults = false` **обязателен**: поля со значениями по умолчанию не нужно передавать. У текущего PATCH и отсутствие
...
- **EAN-13 — это GTIN товарной позиции.** Он одинаков у упаковок одной товарной позиции, включая разные партии; срок годности из ответа
...
нулевой дозе — ошибка; `consume` до нуля даёт `ARCHIVED`; **`isExpiredOn` на границе: годен «до 31 марта» — 31 марта
...
выделение и **не трогает расписания, дозы и дат**; попытка изменить активные дозу или расписание отвергнута; замена создаёт новый курс
...
телом у `recordIntake` и `synchronise`**; пустое тело у операций с обязательным JSON — ошибка; `DELETE`/выход с пустым `204` — успех; битый JSON не роняет
...
вводится и обрабатывается; удаление аптечки с переносом сохраняет пачки; отменённая/невалидная форма не создаёт запись.
...
замена курса сохраняет `TAKEN`, `SKIPPED`, `MISSED` и отменяет будущие `PLANNED`; **активный курс существует без единого доступного источника**;
...
**Тесты:** пороговые даты на фиксированном `Clock`; **курс отменён или источник изменён между показом уведомления и
...
точных будильниках не ломает расписание**; `SNOOZE` не двигает `plannedAt`; обычная пачка не получает предупреждения за 3/1 день,
...
**своя бронь без локального курса показывается отдельно и учитывается в доступности до явного решения**.
...
Курсовой расход идёт через `sync`, внеплановый — `intakes`, нулевая бронь снимается отдельным DELETE.
...
источники, но не удаляет курсы и историю; удаление с переносом сохраняет пачки в целевой аптечке; два клиента видят одинаковые
...
7. **Аналитика.** Прогноз на три месяца, движение за период, внутренние переносы не расход, чужие
   изменения отдельно.
8. **Потеря связи посреди приёма.** Приём офлайн → перезапуск приложения → связь → расход применился не более одного раза; при неустановимом исходе видны факт и запрос сверки.
