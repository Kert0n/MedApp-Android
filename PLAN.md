...
**Направление задано видом движения.** У прихода (`INITIAL`, `TRANSFER_IN`) `delta` неотрицательна,
у расхода (`DISPOSAL`, `TRANSFER_OUT`, `ACCESS_LOST`) неположительна; знаковыми остаются только
`CORRECTION` и `REMOTE_CHANGE` — пересчёт находит и больше, и меньше, а чужое изменение идёт в обе
стороны. Запись против своего вида сходилась бы в сумме, но означала бы обратное написанному.
Перенос внутри одной аптечки отвергается: остаток он не меняет, а в истории выглядел бы событием.
